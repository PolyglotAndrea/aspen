...
import type { Order, OrderItem, OrderType, OrderStatus } from '../../config/tenant.types';
 ...
 * 计算订单金额
 */
function calculateOrderAmount(items: OrderItem[], deliveryFee?: number, packagingFee?: number, discount = 0, pointsAmount = 0): {
  subtotal: number;
  total: number;
} {
 ...
 * 验证订单状态转换是否合法
 */
function canTransitionStatus(current: OrderStatus, target: OrderStatus): boolean {
